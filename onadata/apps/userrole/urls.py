from onadata.apps.userrole.viewsets.UserRoleViewsets import UserRoleViewSet, DonorRoleViewSet, MultiOPSlistViewSet, MultiUserAssignRoleViewSet, MultiUserlistViewSet
...
urlpatterns = [
    ...
    url(r'^api/doners/(?P<pk>\d+)$', DonorRoleViewSet.as_view({'get': 'list'}), name='doner-roles'),
    url(r'^api/people/(?P<level>\d)/(?P<pk>\d+)$', UserRoleViewSet.as_view({'post': 'custom_create','get': 'list'})),
    url(r'^api/multiuserassign/(?P<level>\d)/(?P<pk>\d+)$', MultiUserAssignRoleViewSet.as_view(),
        name="multi_user_assign"),
    url(r'^api/multiuserlist/(?P<level>\d)/(?P<pk>\d+)$', MultiUserlistViewSet.as_view({'get': 'list'}),
        name="multi_user_list"),
    url(r'^api/multi-ops-list/(?P<level>\d)/(?P<pk>\d+)$', MultiOPSlistViewSet.as_view({'get': 'list'}),
        name="multi_ops_list"),
