...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        total_changed = InstanceStatusChanged.objects.filter(finstance__project_id=options['project'], date__lt=last_date).count()
        page_size = 50
        page = 0
        while total_changed > 0:
            time.sleep(2)
            changed = InstanceStatusChanged.objects.filter(finstance__project_id=options['project'],
