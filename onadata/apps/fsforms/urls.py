...
from .views import (
    ...
    save_edumaterial_details,
    setup_project_stages, project_stage_add, Instance_detail,
    alter_answer_status, project_survey,
    project_create_schedule, project_edit_schedule, edit_main_stage,
    edit_sub_stage, edit_schedule, Responses,
    MyOwnFormsListView, share_level, site_general, edit_general,
    project_general, ProjectResponses,
    project_html_export, Deploy_survey, deploy_stages, Deploy_general,
    set_deploy_stages, share_stages,
    edit_share_stages, library_stages, un_deploy_general, un_deploy_survey,
    deploy_general_part, Setup_forms,
    Configure_forms,
    instance_status, Rearrange_stages, deploy_general_remaining_sites,
    delete_substage, delete_mainstage,
    save_educational_material, AlterStatusDetailView, Html_export,
    Project_html_export, AssignFormDefaultStatus,
    FullResponseTable, DeleteMyForm,
    DeleteFInstance,
    FormFillView, CreateKoboFormView, DeleteFieldsightXF,

    FormPreviewView, download_submission, download_xml_version,
    get_attachments_of_finstance, edit_data, view_data,
    FormVersions, repair_mongo, new_data)
...
urlpatterns = urlpatterns + [
    ...
    url(r'^new/(?P<site>\d+)/(?P<form>\d+)$', new_data,
        name='new_data'),
