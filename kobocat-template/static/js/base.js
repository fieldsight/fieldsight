...
function TableViewModel(options, row_model) {
    ...
    var argument = null;
    if (typeof(options.argument) != 'undefined') {
        /** @namespace options.argument */
    ...
    if (typeof row_model != 'undefined') {
        self.rows = ko.observableArray(ko.utils.arrayMap(options.rows, function (item) {
            return new row_model(item, argument);
        }));
        ...
        self.addRow = function () {
            ...
            self.rows.push(new row_model(null, argument));
        };
...
function HashTable() {
    ...
    self.push_lazy = function (item) {
        var matches = self.find(item);
        if (matches) {
            var unique = true;
            for (var i in matches) {
                var match = matches[i];
                if (Object.size(match) == Object.size(item))
                    unique = false;
            }
            if (unique)
                self.push(item);
    ...
    self.find_one = function (params) {
        var result = self.find(params);
        if (result)
            return result[0];
        return null;
    }
