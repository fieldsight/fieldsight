...
if (id_project != 0){
    ...
    webSocketBridgeNotifyUser.connect(channel_url+'project-notify/'+id_project+'/');

}else if (id_organization != 0){

    webSocketBridgeNotifyUser.connect(channel_url+'org-notify/'+id_organization+'/');
   	...
   	//-------------for desktop notification--------------------
		     ...
		     if (Notification.permission !== "granted")
			    ...
			    var notification = new Notification('Notification', {
			      icon: 'https://fieldsight.s3.amazonaws.com/logo/Screenshot_20180201-175449.png',
			      body: text,
			    });
...
// });
		...
		function tasklistgenerate(data, status){
		    ...
		    if (data.status == 2){
		    	if ([3,6,8,9,10,11,12].indexOf(data.task_type) >= 0){
